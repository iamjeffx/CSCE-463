...
char* jump(char* buffer, char* current, char* header, string output, int totalSize, bool DNSAnswer) {
    ...
    if ((unsigned char)current[0] >= 0xC0) {
        ...
        jump(buffer, buffer + offset, header, output, totalSize, DNSAnswer);
        return current + 2;
    }
    // Normal block
    else {
        ...
        current = jump(buffer, current, header, output, totalSize, DNSAnswer);
        return current;
    }
}
        ...
        if ((available = select(0, &fd, NULL, NULL, &timeout)) > 0) {
            ...
            if (htons(fdhRes->questions) > 0) {
                ...
                for (int i = 0; i < htons(fdhRes->questions); i++) {
                    ...
                    while (true) {
                        ...
                        if (blockSize + res - buf > MAX_DNS_SIZE) {
            ...
            if (htons(fdhRes->answers) > 0) {
                ...
                for (int i = 0; i < htons(fdhRes->answers); i++) {
                    ...
                    res = jump(buf, res, resHeader, "", responseBytes, false);
                    ...
                    if (qType == DNS_A) {
                        ...
                        // Perform jump
                        res = jump(buf, res, resHeader, "", responseBytes, true);
                        printf(" TTL = %d\n", 256 * (int)htons(frr->TTL1) + (int)htons(frr->TTL2));
                    }
                }
            }
